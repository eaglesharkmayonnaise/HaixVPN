...
@interface MyOrdersAllViewController ()<UITableViewDelegate,UITableViewDataSource>

@end
// status, 0.Completed, 1.Pending, 2.Failed, 3.Invited  // 4.Free, 5.Invalid
@implementation MyOrdersAllViewController
{
    UITableView *tabelviewALL;
    NSDictionary *OrderListDic;
...
- (void)viewLoad {
    ...
    UIImageView *NullimageALL = [[UIImageView alloc] initWithFrame:CGRectMake(138 *SJwidth, 208 *SJhight - 64, 100, 162)];
    NullimageALL.image = [UIImage imageNamed:@"bottle"];
    [self  addSubview:NullimageALL];
    
    UILabel *NulllabeALL = [[UILabel alloc] initWithFrame:CGRectMake(0, NullimageALL.frame.origin.y + NullimageALL.frame.size.height + 30, kscreenw, 30)];
    ...
    //背景
    UIView *viewback = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kscreenw, kscreenh)];
    ...
    [NetworkApi Getorderlist:nil anddic:nil block:^(NSDictionary *responseObject) {
        [LCProgressHUD hide];
        OrderListDic = responseObject;
        if ([OrderListDic[@"data"][@"orders"] count] >0) {
...
{
    return [OrderListDic[@"data"][@"orders"] count];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    [LCProgressHUD showLoading:nil];
    //获取订单详情
    [NetworkApi Getorderinfo: [NSString stringWithFormat:@"%@",OrderListDic[@"data"][@"orders"][indexPath.section][@"_id"]] anddic:nil block:^(NSDictionary *responseObject) {
        [LCProgressHUD hide];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyOrdersTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (!cell) {
        cell = [[MyOrdersTableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:identifier];
    }

    [cell.labelCompleted setTitle:@"" forState:0];
    [cell.labelCompleted setImage:[UIImage imageNamed:@""] forState:0];
    
    //订单列表
    if ([OrderListDic[@"data"][@"orders"][row][@"status"] intValue] == 0) {
        cell.labeldevices.text = OrderListDic[@"data"][@"orders"][row][@"planName"];
        cell.labelExpires.text = [AppEnvironment UTCchangeDate:OrderListDic[@"data"][@"orders"][row][@"createDateUnixTimestamp"]];
        cell.labelmoney.text = [NSString stringWithFormat:@"¥ %@",OrderListDic[@"data"][@"orders"][row][@"total"]];
    ...
    if ([OrderListDic[@"data"][@"orders"][row][@"status"] intValue] == 1) {
    ...
    if ([OrderListDic[@"data"][@"orders"][row][@"status"] intValue]== 2) {
        [cell.labelCompleted setTitle:@"Paymentfailed" forState:0];
        [cell.labelCompleted setImage:[UIImage imageNamed:@"订单失败"] forState:0];
        [cell.labelCompleted  setTitleColor:[UIColor colorWithRed:1 green:0.48 blue:0.52 alpha:1] forState:0];
    }
    if ([OrderListDic[@"data"][@"orders"][row][@"status"] intValue] == 3) {
