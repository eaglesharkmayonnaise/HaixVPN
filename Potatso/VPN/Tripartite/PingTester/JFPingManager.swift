...
class JFPingManager: NSObject {
    ...
    /// - Returns: 最快的ip
    @objc class func getFastestAddress(addressList: [String], finished: @escaping (_ ipAddress: String?) -> ()) -> () {
...
//            print("最快的地址是:\(fastestAddress) 平均延迟:\(String(format: "%.2lfms", minAvg))")
            print("\(String(format: "%@", fastestAddress)),\(String(format: "%.2lfms", minAvg))")
            finished(String(format: "%@-%.2lfms", fastestAddress,minAvg))
        }
